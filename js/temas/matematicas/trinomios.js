  ...
  extra.raices = function (r) {
    ...
    return {
      guia: G({
        ...
        pasos: [
          { seccion: 'Paso 1: factorizar',
            queHacemos: 'Antes de resolver nada, factorizamos el trinomio.',
            paraQue: 'Una ecuacion suelta no se puede partir; un PRODUCTO igualado a cero si.',
            queda: (-p) + ' y ' + (-q),
          ...
          { seccion: 'Paso 2: usar el producto cero',
            queHacemos: 'Aplicamos que un producto vale 0 solo si algun factor vale 0.',
            paraQue: 'Asi una ecuacion de grado 2 se parte en dos ecuaciones de grado 1, que son faciles.',
            queda: fac(1, -p) + ' = 0   o   ' + fac(1, -q) + ' = 0',
          ...
          { seccion: 'Paso 3: primera solucion',
            queHacemos: 'Despejamos la x del primer factor.',
            paraQue: 'Pasamos el numero al otro lado cambiandole el signo.',
            queda: 'x = ' + p,
          ...
          { seccion: 'Paso 4: segunda solucion',
            queHacemos: 'Lo mismo con el otro factor.',
            paraQue: 'Cada factor da una solucion: por eso hay dos.',
            queda: 'x = ' + p + ',  x = ' + q,
          ...
          { seccion: 'Paso 5: escribir',
            ...
            paraQue: 'Se pueden comprobar sustituyendolas en la ecuacion original.',
            queda: 'x = ' + p + ',  x = ' + q,
      ...
      guia: G({
        ...
        pasos: [
          { seccion: 'Paso 1: la forma',
            queHacemos: 'Decidimos como se ven los dos parentesis.',
            paraQue: 'El primer termino es x&sup2;, asi que cada uno empieza con x; el ultimo lleva y&sup2;, asi que cada uno termina con y.',
            queda: '(x + ?y)(x + ?y)',
          ...
          { seccion: 'Paso 2: los dos numeros',
            queHacemos: 'Buscamos dos numeros con producto ' + c + ' y suma ' + b + '.',
            paraQue: 'Que haya una y no cambia el metodo: la y solo se arrastra pegada a cada numero.',
            queda: p + ' y ' + q,
          ...
          { seccion: 'Paso 3: escribir',
            queHacemos: 'Escribimos los dos parentesis con su y.',
      ...
      if (dif === 'facil') {
        ...
        if (t === 'conCoef') {
          ...
          guiaDelPaso = G({
            ...
            pasos: [
              { seccion: 'Paso 1: calcular a&middot;c',
                queHacemos: 'Multiplicamos el primer coeficiente por el ultimo.',
                paraQue: 'Con a distinto de 1 el truco de siempre no sirve. El metodo ac arranca con este numero.',
                queda: 'a&middot;c = ' + (pol[0] * pol[2]),
              ...
              { seccion: 'Paso 2: los dos numeros',
                queHacemos: 'Buscamos dos numeros con producto a&middot;c y suma b.',
                paraQue: 'Con ellos se parte el termino de en medio en dos, y asi se puede agrupar.',
                queda: F.une([F.term(pol[0], 'x', 2), F.term(m * q, 'x', 1), F.term(n * p, 'x', 1), String(pol[2])]),
              ...
              { seccion: 'Paso 3: agrupar de dos en dos',
                queHacemos: 'Sacamos factor comun de los dos primeros terminos.',
                paraQue: 'Partido en dos parejas, cada pareja si tiene factor comun.',
                queda: F.term(m, 'x', 1) + '(' + F.poli([n, q], 'x') + ') + [' + F.une([F.term(n * p, 'x', 1), String(pol[2])]) + ']',
              ...
              { seccion: 'Paso 3: agrupar de dos en dos',
                queHacemos: 'Ahora el factor comun de los dos ultimos.',
                paraQue: 'Si dentro queda el MISMO parentesis que antes, el metodo funciono.',
                queda: F.term(m, 'x', 1) + '(' + F.poli([n, q], 'x') + ') ' + (p > 0 ? '+ ' + p : '&minus; ' + (-p)) + '(' + F.poli([n, q], 'x') + ')',
              ...
              { seccion: 'Paso 4: escribir',
                queHacemos: 'Sacamos el parentesis repetido como factor comun.',
                paraQue: 'Un parentesis es el que se repetia y el otro se arma con lo que sacaste.',
                queda: fac(m, p) + fac(n, q),
          ...
          guiaDelPaso = G({
            ...
            pasos: [
              { seccion: 'Paso 1: las dos raices',
                queHacemos: 'Sacamos la raiz cuadrada del primer termino.',
                paraQue: 'Si sale de (a + b)&sup2;, el primer termino es a&sup2;: su raiz es la a que buscamos.',
                queda: '(' + F.term(c, 'x', 1) + ' ? ?)&sup2;',
              ...
              { seccion: 'Paso 1: las dos raices',
                queHacemos: 'Ahora la raiz del ultimo termino.',
                paraQue: 'Esa es la b. El signo se decide aparte, por eso va sin signo por ahora.',
                queda: '(' + F.term(c, 'x', 1) + ' ? ' + Math.abs(a) + ')&sup2;',
              ...
              { seccion: 'Paso 2: comprobar',
                queHacemos: 'Calculamos 2&middot;a&middot;b y lo comparamos con el termino de en medio.',
                paraQue: 'Este paso NO se salta: si no coincide, el trinomio no es cuadrado perfecto y este metodo daria mal.',
                queda: F.term(pol[1], 'x', 1) + ' coincide &check;',
              ...
              { seccion: 'Paso 3: el signo',
                queHacemos: 'Elegimos el signo del binomio.',
                paraQue: 'Es el mismo signo que trae el termino de en medio. El ultimo termino siempre sale positivo, asi que no dice nada.',
                queda: fac(c, a) + '&sup2;',
                ...
                despues: '' },
              { seccion: 'Paso 4: escribir',
                queHacemos: 'Escribimos el binomio al cuadrado.',
                paraQue: 'Se puede comprobar desarrollandolo: debe volver al trinomio original.',
                queda: fac(c, a) + '&sup2;',
                pregunta: 'Escribe la factorizacion completa.',
          ...
          guiaDelPaso = G({
            ...
            pasos: [
              { seccion: 'Paso 1: la mitad de b',
                queHacemos: 'Partimos a la mitad el coeficiente de en medio.',
                paraQue: 'La prueba es c = (b/2)&sup2;. Primero hace falta esa mitad.',
                queda: 'b/2 = ' + (pol[1] / 2),
                ...
                despues: '' },
              { seccion: 'Paso 2: elevarla al cuadrado',
                queHacemos: 'Elevamos al cuadrado la mitad.',
                paraQue: 'Ese es el valor que DEBERIA tener el ultimo termino si fuera cuadrado perfecto.',
                queda: 'deberia ser ' + ((pol[1] / 2) * (pol[1] / 2)),
              ...
              { seccion: 'Paso 3: leer el ultimo termino',
                queHacemos: 'Miramos cuanto vale de verdad el ultimo termino.',
                paraQue: 'Para poder comparar el valor real contra el que deberia ser.',
                queda: ((pol[1] / 2) * (pol[1] / 2)) + '  contra  ' + pol[2],
              ...
              { seccion: 'Paso 4: comparar y decidir',
                queHacemos: 'Comparamos los dos numeros y decidimos.',
                paraQue: 'Si son iguales es cuadrado perfecto; si no, hay que factorizarlo por otro camino.',
                queda: esPerfecto ? 'Si: ' + fac(1, a) + '&sup2;' : 'No es cuadrado perfecto',
        ...
        if (t2 === 'factorComun') {
          ...
          guiaDelPaso = G({
            ...
            pasos: [
              { seccion: 'Paso 1: buscar el factor comun',
                queHacemos: 'Buscamos el numero que divide a los tres coeficientes.',
                paraQue: 'Factorizar COMPLETAMENTE empieza siempre por aqui. Si te lo saltas, la respuesta queda a medias.',
                queda: String(k),
              ...
              { seccion: 'Paso 2: sacarlo',
                queHacemos: 'Dividimos cada termino entre el factor comun.',
                paraQue: 'Adentro queda un trinomio limpio, con la x&sup2; sin coeficiente: ese si se factoriza a la vieja usanza.',
                queda: k + '(' + P.texto(P.escala(pol, 1 / k)) + ')',
              ...
              { seccion: 'Paso 3: factorizar lo de adentro',
                queHacemos: 'Buscamos los dos numeros del trinomio de adentro.',
                paraQue: 'Producto = el termino sin x, suma = el coeficiente de la x.',
                queda: (-p) + ' y ' + (-q),
              ...
              { seccion: 'Paso 4: escribir',
                queHacemos: 'Juntamos el factor comun y los dos parentesis.',
                paraQue: 'Olvidar el numero de afuera es el error mas comun de este tipo.',
                queda: k + fac(1, -p) + fac(1, -q),
          ...
          guiaDelPaso = G({
            ...
            pasos: [
              { seccion: 'Paso 1: el cambio de variable',
                queHacemos: 'Cambiamos x&sup2; por u y vemos en que se convierte x&#8308;.',
                paraQue: 'Los exponentes son 4, 2 y 0: el doble, el sencillo y nada. Con el cambio se vuelve un trinomio normal.',
                queda: 'u&sup2; &minus; ' + (p + q) + 'u + ' + (p * q),
              ...
              { seccion: 'Paso 2: factorizar en u',
                queHacemos: 'Factorizamos el trinomio en u como cualquier otro.',
                paraQue: 'Ya es x&sup2; + bx + c de toda la vida, solo que la letra se llama u.',
                queda: '(u &minus; ' + p + ')(u &minus; ' + q + ')',
              ...
              { seccion: 'Paso 3: deshacer el cambio',
                queHacemos: 'Recordamos que la u era solo un apodo.',
                paraQue: 'La pregunta era en x. Dejar la respuesta en u es dejarla a medio terminar.',
                queda: '(u &minus; ' + p + ')(u &minus; ' + q + ')  con u = x&sup2;',
              ...
              { seccion: 'Paso 4: escribir',
                queHacemos: 'Cambiamos cada u por x&sup2;.',
                paraQue: 'Se puede comprobar multiplicando de vuelta.',
                queda: '(x&sup2; &minus; ' + p + ')(x&sup2; &minus; ' + q + ')',
          ...
          guiaDelPaso = G({
            ...
            pasos: [
              { seccion: 'Paso 1: encontrar h',
                queHacemos: 'Partimos a la mitad el coeficiente de la x.',
                paraQue: 'Al desarrollar (x + h)&sup2; el termino de en medio es 2h. Para que de b, h tiene que ser b/2.',
                queda: '(x ' + (h < 0 ? '&minus; ' + Math.abs(h) : '+ ' + h) + ')&sup2; + k',
              ...
              { seccion: 'Paso 2: ver que trae de regalo',
                queHacemos: 'Calculamos h&sup2;, el numero que aparece solo al desarrollar.',
                paraQue: 'Ese h&sup2; no lo pedimos: llega solo. Por eso casi nunca coincide con la c del original.',
                queda: 'el binomio trae ' + (h * h) + ', pero hace falta ' + c,
              ...
              { seccion: 'Paso 3: ajustar con k',
                queHacemos: 'Restamos para ver cuanto falta o cuanto sobra.',
                paraQue: 'k es justo la correccion: k = c &minus; h&sup2;. Con ella la expresion vuelve a valer lo mismo que antes.',
                queda: '(x ' + (h < 0 ? '&minus; ' + Math.abs(h) : '+ ' + h) + ')&sup2; ' + (kk < 0 ? '&minus; ' + Math.abs(kk) : '+ ' + kk),
              ...
              { seccion: 'Paso 4: escribir',
                queHacemos: 'Damos h y k.',
                paraQue: 'Se comprueba desarrollando: debe volver al trinomio original.',
                queda: '(x ' + (h < 0 ? '&minus; ' + Math.abs(h) : '+ ' + h) + ')&sup2; ' + (kk < 0 ? '&minus; ' + Math.abs(kk) : '+ ' + kk),
