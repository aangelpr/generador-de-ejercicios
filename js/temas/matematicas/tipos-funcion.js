...
(function () {
  ...
  extra.tabla = function (r) {
    ...
    var pasosT = [
      { seccion: 'Paso 1: probar restando',
        queHacemos: 'Restamos cada valor con el anterior.',
        paraQue: 'Siempre se empieza por lo mas barato. Si las diferencias salen constantes, ya esta: es lineal.',
        queda: 'diferencias: ' + (vals[1] - vals[0]) + ', ?',
      ...
      { seccion: 'Paso 1: probar restando',
        queHacemos: 'Calculamos la siguiente diferencia.',
        paraQue: 'Con dos ya se puede ver si coinciden.',
        queda: 'diferencias: ' + (vals[1] - vals[0]) + ', ' + (vals[2] - vals[1]),
      ...
      { seccion: 'Paso 2: leer el resultado',
        queHacemos: 'Comparamos las diferencias entre si.',
        paraQue: 'Diferencias constantes = LINEAL, y esa diferencia es la pendiente. Si no, toca seguir investigando.',
        queda: tipo === 0 ? 'lineal' : 'no es lineal; hay que probar dividiendo',
    ...
    if (tipo !== 0) {
      pasosT.push({
        seccion: 'Paso 3: probar dividiendo',
        queHacemos: 'Dividimos cada valor entre el anterior.',
        paraQue: 'Si al restar no salia constante, quiza al dividir si: eso pasa en las exponenciales.',
        queda: 'cocientes: ' + F.n(vals[1] / vals[0], 2) + ', ' + F.n(vals[2] / vals[1], 2),
      ...
      pasosT.push({
        ...
        queHacemos: 'Comparamos los cocientes.',
        paraQue: 'Cociente constante = EXPONENCIAL. Si tampoco, se mira la diferencia DE LAS diferencias: si esa es constante, es cuadratica.',
        queda: tipo === 2 ? 'exponencial' : 'cuadratica',
    ...
    pasosT.push({
      seccion: 'Paso 5: decidir',
      queHacemos: 'Damos el tipo.',
      paraQue: 'Diferencias constantes: lineal. Segundas diferencias constantes: cuadratica. Cocientes constantes: exponencial.',
      queda: ['Lineal', 'Cuadratica', 'Exponencial'][tipo],
  ...
  extra.evaluarCompuesta = function (r) {
    ...
    return {
      guia: G({
        ...
        pasos: [
          { seccion: 'Paso 1: por donde empezar',
            queHacemos: 'Decidimos que funcion se aplica primero.',
            paraQue: 'Igual que con cualquier parentesis: primero lo de mas adentro. No se puede aplicar f hasta saber que numero meterle.',
            queda: 'f(g(' + x0 + ')) = ?,  g(f(' + x0 + ')) = ?',
            ...
            despues: '' },
          { seccion: 'Paso 2: la de adentro',
            queHacemos: 'Evaluamos g en el numero.',
            paraQue: 'Ese resultado es lo que se le va a meter a f.',
            queda: 'g(' + x0 + ') = ' + gx + ';  f(g(' + x0 + ')) = ?',
          ...
          { seccion: 'Paso 3: la de afuera',
            queHacemos: 'Metemos ese resultado en f.',
            paraQue: 'Con eso queda resuelta la primera composicion.',
            queda: 'f(g(' + x0 + ')) = ' + fgx + ';  g(f(' + x0 + ')) = ?',
          ...
          { seccion: 'Paso 4: la composicion al reves',
            queHacemos: 'Ahora la de adentro es f.',
            paraQue: 'Misma idea, pero cambiando el orden. Otra vez de adentro hacia afuera.',
            queda: 'f(g(' + x0 + ')) = ' + fgx + ';  f(' + x0 + ') = ' + (a * x0 + b),
            pregunta: 'Ahora al reves. Empieza por dentro otra vez: f(' + x0 + ')',
            resp: R.numero(a * x0 + b, { dec: 2 }),
            pista: a + '(' + x0 + ') ' + (b < 0 ? '&minus; ' + (-b) : '+ ' + b) + '.',
            despues: '' },
          { seccion: 'Paso 4: la composicion al reves',
            queHacemos: 'Y ese resultado se lo metemos a g.',
            paraQue: 'Con eso queda la segunda composicion.',
            queda: 'f(g(' + x0 + ')) = ' + fgx + ',  g(f(' + x0 + ')) = ' + gfx,
            pregunta: 'Y aplicale g: g(' + (a * x0 + b) + ')',
            resp: R.numero(gfx, { dec: 2 }),
            pista: c + '(' + (a * x0 + b) + ') ' + (d < 0 ? '&minus; ' + (-d) : '+ ' + d) + '.',
            despues: '' },
          { seccion: 'Paso 5: comparar',
            queHacemos: 'Ponemos los dos resultados al lado.',
            paraQue: 'La composicion NO es conmutativa: f(g(x)) y g(f(x)) casi nunca dan lo mismo. Por eso hay que leer con cuidado cual va adentro.',
            queda: fgx === gfx ? 'coincidieron, pero es casualidad' : 'distintos: el orden SI importa',
      ...
      if (dif === 'facil' || dif === 'medio') {
        ...
        if (t2 === 'inversa') {
          ...
          guiaDelPaso = G({
            ...
            pasos: [
              { seccion: 'Paso 1: intercambiar x con y',
                queHacemos: 'Escribimos y = f(x) e intercambiamos las letras.',
                paraQue: 'Invertir una funcion es intercambiar los papeles de entrada y salida. Por eso se intercambian las letras.',
                queda: 'x = ' + a + 'y ' + (b < 0 ? '&minus; ' + (-b) : '+ ' + b),
              ...
              { seccion: 'Paso 2: deshacer la suma',
                queHacemos: 'Pasamos el termino independiente al otro lado.',
                paraQue: 'Se deshacen las operaciones en orden INVERSO: primero la suma, luego la multiplicacion.',
                queda: 'x ' + (b < 0 ? '+ ' + (-b) : '&minus; ' + b) + ' = ' + a + 'y',
              ...
              { seccion: 'Paso 3: deshacer la multiplicacion',
                queHacemos: 'Dividimos entre el coeficiente.',
                paraQue: 'TODO el numerador se divide, por eso va el parentesis.',
                queda: 'f&#8315;&sup1;(x) = ' + F.frac('x ' + (b < 0 ? '+ ' + (-b) : '&minus; ' + b), a),
              ...
              { seccion: 'Paso 4: comprobar',
                queHacemos: 'Componemos las dos funciones.',
                paraQue: 'Si una deshace a la otra, aplicarlas seguidas te deja donde empezaste. Ademas sus graficas son espejos respecto a y = x.',
                queda: 'f&#8315;&sup1;(x) = ' + F.frac('x ' + (b < 0 ? '+ ' + (-b) : '&minus; ' + b), a),
          ...
          guiaDelPaso = G({
            ...
            pasos: [
              { seccion: 'Paso 1: que es componer',
                queHacemos: 'Metemos toda la g dentro de la x de f.',
                paraQue: 'Componer NO es multiplicar. La g se convierte en la entrada de la f. Y ojo con los PARENTESIS: sin ellos sale mal.',
                queda: a + '(' + P.texto([c, d]) + ') ' + (b < 0 ? '&minus; ' + (-b) : '+ ' + b),
              ...
              { seccion: 'Paso 2: distribuir',
                queHacemos: 'Multiplicamos el coeficiente por cada termino de adentro.',
                paraQue: 'Este es el paso donde mas se falla: multiplicar solo el primer termino y olvidar el segundo.',
                queda: P.texto(P.escala([c, d], a)) + ' ' + (b < 0 ? '&minus; ' + (-b) : '+ ' + b),
              ...
              { seccion: 'Paso 3: juntar',
                queHacemos: 'Sumamos los terminos independientes.',
                paraQue: 'Comprobacion: si evaluas esta funcion en un numero, debe dar lo mismo que hacer g primero y f despues.',
                queda: 'f(g(x)) = ' + P.texto(comp),
          ...
          guiaDelPaso = G({
            ...
            pasos: [
              { seccion: 'Paso 1: la prueba',
                queHacemos: 'Recordamos en que consiste la prueba.',
                paraQue: 'Una recta horizontal es "todos los puntos con la misma y". Si toca la curva dos veces, hay dos x distintas con la misma imagen.',
                queda: 'hay que buscar valores repetidos',
              ...
              { seccion: 'Paso 2: aplicarla',
                queHacemos: 'Buscamos si esta funcion repite valores.',
                paraQue: 'Aqui ' + cs.por + '.',
                queda: cs.iny ? 'no repite valores' : 'si repite valores',
              ...
              { seccion: 'Paso 3: decidir',
                ...
                paraQue: 'Importa porque solo las funciones inyectivas tienen inversa.',
                queda: cs.iny ? 'es inyectiva' : 'no es inyectiva',
